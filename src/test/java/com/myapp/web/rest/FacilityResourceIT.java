...
import com.myapp.repository.FacilityRepository;
import com.myapp.service.criteria.FacilityCriteria;
...
class FacilityResourceIT {
    ...
    @Test
    @Transactional
    void getAllFacilitiesByRoomidIsEqualToSomething() throws Exception {
        // Initialize the database
        facilityRepository.saveAndFlush(facility);
        Room roomid;
        if (TestUtil.findAll(em, Room.class).isEmpty()) {
            roomid = RoomResourceIT.createEntity(em);
            em.persist(roomid);
            em.flush();
        } else {
            roomid = TestUtil.findAll(em, Room.class).get(0);
        }
        em.persist(roomid);
        em.flush();
        facility.setRoomid(roomid);
        facilityRepository.saveAndFlush(facility);
        Long roomidId = roomid.getId();

        // Get all the facilityList where roomid equals to roomidId
        defaultFacilityShouldBeFound("roomidId.equals=" + roomidId);

        // Get all the facilityList where roomid equals to (roomidId + 1)
        defaultFacilityShouldNotBeFound("roomidId.equals=" + (roomidId + 1));
    }
